...
class ThresholdSelectionStrategy(object):
    ...
    def select_threshold(self, points: List[Any], origin: Any, dist: Dist) -> float:
        ...
        :return: a partitioning threshold distance appropriate for the given list of points; ideally, some points should be closer to the origin than the returned threshold, and some should be farther
        """
...
class MedianDistanceThresholdSelectionStrategy(ThresholdSelectionStrategy):
    ...
    def select_threshold(self, points: List[Any], origin: Any, dist: Dist) -> float:
        """
        Returns the median distance of the given points from the given origin. This method will partially sort the list of points in the process.
...
class SamplingMedianDistanceThresholdSelectionStrategy(MedianDistanceThresholdSelectionStrategy):
    ...
    def select_threshold(self, points: List[Any], origin: Any, dist: Dist) -> float:
        """
        Returns the median distance of a subset of the given points from the given origin. The given list of points may be partially sorted in the process.
        ...
        :return: the median distance from the origin to the given list of points
        """
        return MedianDistanceThresholdSelectionStrategy.select_threshold(self, self._get_sampled_points(points), origin, dist)
...
class NearestNeighborCollector(object):
    """
    A utility class that uses a priority queue to efficiently collect results for a k-nearest-neighbors query in a vp-tree.
    """
    def __init__(self, query_point: Any, dist: Dist, capacity: int):
        ...
        :param dist: the distance function to be used to determine the distance between the query point and potential neighbors
    ...
    def offer_point(self, point: Any) -> None:
        """
        ...
        """
        point_added = False
        distance_to_new_point = self.dist.dist(self.query_point, point)
        queue_item = (-distance_to_new_point, point)
        if len(self.priority_queue) < self.capacity:
            heapq.heappush(self.priority_queue, point)
            point_added = True
        else:
    ...
    def to_sorted_list(self) -> List[Tuple[Any, float]]:
        ...
        """
        return [(i[1], -i[0]) for i in self.priority_queue[::-1]]
...
class VPTreeNode(object):
    ...
    """
    def __init__(self, points: List[Any], dist: Dist, threshold_selection_strategy: ThresholdSelectionStrategy, capacity: int):
        """
        ...
        :param capacity: the desired maximum capacity of this node; this node may contain more points than the given capacity if the given collection of points cannot be partitioned (for example, because all of the points are an equal distance away from the vantage point)
        """
        self.capacity = capacity
        self.dist = dist
    ...
    def _partition_points(self) -> int:
        ...
        :return: the index of the first point in the list that falls beyond the distance threshold or -1 if the list of points could not be partitioned (i.e. because they are all the same distance from the vantage point
        """
        left = 0
    ...
    def anneal(self) -> None:
        ...
        if self.points is None:
            ...
            # threshold point.
            self.threshold = self.threshold_selection_strategy.select_threshold(self.points, self.vantage_point, self.dist)
            ...
            if first_index_past_threshold == -1:
                ...
                self.closer = VPTreeNode(self.points[:first_index_past_threshold], self.dist, self.threshold_selection_strategy, self.capacity)
                self.farther = VPTreeNode(self.points[first_index_past_threshold:], self.dist, self.threshold_selection_strategy, self.capacity)
                self.points = None
    ...
    def get_child_node_for_point(self, point: any) -> 'VPTreeNode':
        ...
        :param point: the point for which to choose an appropriate child node; the point need not actually exist within either child node
        :return: this node's "closer" child node if the given point is within this node's distance threshold of the vantage point or the "farther" node otherwise
        """
    ...
    **O(log(n))** time.
    ...
    def get_nearest_neighbors(self, query_point: Any, max_results: int = 10) -> Union[List[Tuple[Any, float]], None]:
        ...
        *max_results* is larger than the number of points in the index). If multiple points have the same distance
        ...
        :return: a list of the nearest neighbors to the given query point sorted by increasing distance from the query point
        """
